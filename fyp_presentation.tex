...
\begin{frame}
	...
	\framesubtitle{Three active research lines, one missing connection}
...
\begin{frame}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.56\textwidth}
			...
			{\tiny
			\[
			\bar{\mathbf e}(d)=\operatorname{PCHIP}
			\!\left(d_j,\left|\operatorname{smooth}_{21}(\mathbf e_j)\right|\right),
			\qquad \mathbf e_j=\mathrm{GT}_{A,j}-\mathrm{base}_{A,j}
			\]
			\[
			\mathbf r=\max\!\left(0,\left|\Delta\mathbf T_B\right|-\bar{\mathbf e}(d_B)\right)
			\]
			\textit{Variables:} \(d_j\) is accumulated Pass A baseline XY distance; \(\mathbf e_j=[e_x,e_y,e_z,e_\psi]\) is its aligned GT correction; \(\bar{\mathbf e}(d)\) is the expected absolute correction; \(\Delta\mathbf T_B\) is a Pass B candidate minus its live drifted pose.
			\vspace{0.4mm}
			\begin{itemize}
				\setlength{\itemsep}{0.25mm}
				\item Align Pass A baseline and GT, then smooth over 21 samples (about 10.7 s or 1.5 m): enough to suppress local jitter while retaining accumulated drift.
				\item Fit a component-wise PCHIP curve against travelled XY distance and query it at the current Pass B distance.
				\item Manual transfer slack on the \emph{excess}, not the total correction: 3.0 m XY, 2.5 m z, and 0.70 rad yaw.
				\item Absolute magnitudes transfer across route direction; NAS-GS score still selects the final pose.
			\end{itemize}
			}
		\end{column}
	\end{columns}
...
\begin{frame}
	...
	\framesubtitle{151216 wind-turbine inspection sequence}

	\begin{columns}[T]
		\begin{column}{0.63\textwidth}
			\begin{itemize}\small
				\setlength{\itemsep}{0.8mm}
				\item Real 151216 sequence recorded around an offshore wind-turbine foundation with a Tritech Gemini 1200ik imaging sonar.
...
\begin{frame}
	...
	\begin{columns}[T]
		\begin{column}{0.48\textwidth}
			...
			\begin{itemize}
				\item Reverse-route revisit remains sensitive to route-progress endpoints and large drift.
				\item Wide candidate searches require stronger temporal evidence before large corrections are safe.
				\item Offshore evaluation uses Aqua-SLAM as the reference rather than independent survey ground truth.
			\end{itemize}
		\end{column}
		\begin{column}{0.48\textwidth}
			...
			\begin{itemize}
				\item Develop sequence-level optimisation for robust large-drift recovery.
				\item Stabilise route-progress estimation at reverse-route boundaries.
				\item Retrain NAS-GS using corrected trajectories and validate against an independent reference.
				\item Progress from live replay to onboard timing, calibration, and safety trials.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
